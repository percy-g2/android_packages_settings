...
public final class BluetoothNameDialogFragment extends DialogFragment implements TextWatcher {
    ...
    public void onResume() {
        ...
        mDeviceNameView.setFocusable(true);
        mDeviceNameView.requestFocus();
        mDeviceNameView.setFocusableInTouchMode(true);
        new Timer(true).schedule(new TimerTask() {
            public void run() {
                InputMethodManager inputManager =
                        (InputMethodManager) mDeviceNameView.getContext().getSystemService(
                                Context.INPUT_METHOD_SERVICE);
                inputManager.showSoftInput(mDeviceNameView, 0);
            }
        }, 500);
    }
    ...
    public void afterTextChanged(Editable s) {
        if (mDeviceNameUpdated) {
            ...
            if (mOkButton != null) {
                boolean enabled = (s.length() != 0) && (s.toString().trim().length() != 0);
                mOkButton.setEnabled(enabled);
                if (!enabled) mDeviceNameEdited = false;
            }
        }
    }
