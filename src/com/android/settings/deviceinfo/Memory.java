...
public class Memory extends SettingsPreferenceFragment {
    ...
    public void onCreate(Bundle icicle) {
        ...
        for(int i = 0; i < length; i++){
            if(storageVolumes[i].allowMassStorage())
            {
                mTempLength++;
            }
        }
        mStorageVolumePreferenceCategories = new StorageVolumePreferenceCategory[mTempLength];
        for (int i = 0; i < length; i++) {
            ...
            if ("1".equals(SystemProperties.get("persist.sys.emmcsdcard.enabled"))) {
                ...
                storageVolume = storageVolumes[length - 1 - i];
            }
            if(!storageVolume.allowMassStorage()){
                // Do not display the storage menu which are unable to mount
                continue;
            }
            boolean isPrimary = false ; //don't display the percentage details for USB & SD storage
    ...
    private final BroadcastReceiver mMediaScannerReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            Log.i(TAG, "Received Broadcast Event " + action);
            if (action.equals(Intent.ACTION_MEDIA_SCANNER_FINISHED)) {
                ...
                for (int i = 0; i < mStorageVolumePreferenceCategories.length; i++) {
                    mStorageVolumePreferenceCategories[i].onMtpStorageStateChanged();
                }
            }
        }
    };
