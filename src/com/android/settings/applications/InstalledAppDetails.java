...
public class InstalledAppDetails extends Fragment
    ...
    // When calling onResume, record the time.
    ...
    private void initMoveButton() {
        ...
        if (dataOnly) {
            ...
            String storageValue = SystemProperties.get("persist.sys.emmcsdcard.enabled");
       
            if ("1".equals(storageValue)) {
                mMoveAppButton.setText(R.string.move_app_to_sdcard_nosdcard);
            }
            else {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        if ("1".equals(storageValue)) {
            ...
            mExternalCodeSize = (TextView)view.findViewById(R.id.external_data_size_text);
            mExternalDataSize = (TextView)view.findViewById(R.id.external_code_size_text);
        }
    ...
    public void onResume() {
        ...
        // get the current time when calling onResume.
        getCurrentTime();
    }
    ...
    public void onStop() {
        ...
        super.onStop();
        listenerHomekeyThread();
    }

    // Monitoring Home key event by the thread.
    // when press Home key and judge whether the current activity's componentName is InstalledAppDetails,
    // if true, InstalledAppDetails finish.
    private void listenerHomekeyThread(){
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                try{
                    String str = "logcat -v time ActivityManager:I *:S";
                    mLogcatProc = Runtime.getRuntime().exec(str);
                    reader = new BufferedReader(new InputStreamReader(mLogcatProc.getInputStream()));
                    String line;
                    String pressHomeTime;
                    while((line = reader.readLine()) != null){
                        // Monitoring the Home key event.
                        if(line.indexOf("android.intent.category.HOME") > 0){
                            pressHomeTime =line.substring(0, line.indexOf("."));
                            if(compareDate(pressHomeTime,currentTime)){
                                // judge whether the current activity's componentName is InstalledAppDetails.
                                if (getActivity().getComponentName().getClassName().equals("com.android.settings.applications.InstalledAppDetails"))
                                {
    ...
    // compare whether Date1 is later than Date2.
    ...
    private void getCurrentTime(){
        Time t = new Time();
        t.setToNow();
        String month = ((t.month+1 < 10)?"0"+Integer.toString(t.month+1):Integer.toString(t.month+1));
        String day = ((t.monthDay < 10)?"0"+Integer.toString(t.monthDay):Integer.toString(t.monthDay));
        String hour = ((t.hour < 10)?"0"+Integer.toString(t.hour):Integer.toString(t.hour));
        String minute = ((t.minute < 10)?"0"+Integer.toString(t.minute):Integer.toString(t.minute));
        String second = ((t.second < 10)?"0"+Integer.toString(t.second):Integer.toString(t.second));
        currentTime = month+"-"+day+" "+hour+":"+minute+":"+second;
    }
    ...
    public static class MyAlertDialogFragment extends DialogFragment {
        ...
        private void notifyAudioDeviceUnavailable(){
            if(getOwner().mAppEntry.info.packageName.equals("com.quicinc.fmradio")){
            ...
            switch (id) {
                ...
                case DLG_FORCE_STOP:
                    ...
                    .setPositiveButton(R.string.dlg_ok,
                        new DialogInterface.OnClickListener() {
                        public void onClick(DialogInterface dialog, int which) {
                             // If we force close the VideoCall, the camera had released, so we
                             // can not back to the VideoCallScreen. In the new design, if we force
                             // close the video call, we prefer to hangup the videocall.
                             if (getOwner().mAppEntry.info.packageName.equals("com.borqs.videocall")) {
                                  Intent intent = new Intent(GsmCallTracker.FORCE_CLOSE_VIDEO_CALL);
                                  getActivity().sendBroadcast(intent);
                             } else if(getOwner().mAppEntry.info.packageName.equals("com.quicinc.fmradio")){
                                int speakerEnabled = AudioSystem.getForceUse(AudioSystem.FOR_MEDIA);
                                if (speakerEnabled == AudioSystem.FORCE_SPEAKER) {
                ...
                CharSequence msg = getActivity().getString(
                        R.string.move_app_failed_dlg_text,
                ...
                return new AlertDialog.Builder(getActivity())
                        .setTitle(
                                getActivity().getText(
                                        R.string.move_app_failed_dlg_title))
                        .setIcon(android.R.drawable.ic_dialog_alert)
                        .setMessage(msg)
                        .setNeutralButton(R.string.dlg_ok, null).create();
                case DLG_DISABLE:
                    return new AlertDialog.Builder(getActivity())
                    ...
                    .setPositiveButton(R.string.dlg_ok,
                        ...
                        public void onClick(DialogInterface dialog, int which) {
                            ...
                            .execute((Object)null);
                            if((getOwner().mAppEntry.info.packageName).equals("com.android.soundrecorder")) {
                                Intent i = new Intent("com.quicinc.fmradio.DISABLE_SOUNDRECORDER");
