...
public class RingerVolumePreference extends VolumePreference {
    ...
    private RingerVolumePreferenceSeekbar [] mSeekBarVolumizer;
    ...
    private Runnable refreshViewState = new Runnable(){
        @Override
        public void run(){
            doItInRunnable();
        }
        ...
        for (int i = 0; i < SEEKBAR_TYPE.length; i++) {
            ...
            if (mSeekBars[i] != null) {
               if (SEEKBAR_TYPE[i] == AudioManager.STREAM_MUSIC){
                   mSeekBarVolumizer[i].initSeekBarAfterDisplay(mSeekBars[i],getMediaVolumeUri(getContext()));
               }else{
                   mSeekBarVolumizer[i].initSeekBarAfterDisplay(mSeekBars[i],null);
               }
    ...
    public RingerVolumePreference(Context context, AttributeSet attrs) {
        ...
        mSeekBarVolumizer = new RingerVolumePreferenceSeekbar[SEEKBAR_ID.length];
    ...
    protected void onBindDialogView(View view) {
        ...
        for (int i = 0; i < SEEKBAR_ID.length; i++) {
            ...
            mSeekBars[i] = seekBar;
            mSeekBarVolumizer[i] = new RingerVolumePreferenceSeekbar(getContext(), seekBar,
                        SEEKBAR_TYPE[i]);
        }
        ...
        new Thread(refreshViewState).start();
    ...
    protected void onDialogClosed(boolean positiveResult) {
        ...
        if (!positiveResult) {
            for (RingerVolumePreferenceSeekbar vol : mSeekBarVolumizer) {
    ...
    protected Parcelable onSaveInstanceState() {
        ...
        for (int i = 0; i < SEEKBAR_ID.length; i++) {
            RingerVolumePreferenceSeekbar vol = mSeekBarVolumizer[i];
            if (vol != null) {
    ...
    public class RingerVolumePreferenceSeekbar extends SeekBarVolumizer{
        public RingerVolumePreferenceSeekbar(Context context, SeekBar seekBar, int streamType, Uri defaultUri) {
            super(context, seekBar, streamType, defaultUri);
        }
        public RingerVolumePreferenceSeekbar(Context context, SeekBar seekBar, int streamType) {
            super(context, seekBar, streamType);
        }
        @Override
        public void initSeekBar(SeekBar seekBar, Uri defaultUri){
            initSeekBarBeforeDisplay(seekBar, defaultUri);
        }
   }
