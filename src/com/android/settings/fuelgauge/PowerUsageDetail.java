...
public class PowerUsageDetail extends Fragment implements Button.OnClickListener {
        ...
        switch (action) {
            ...
            case ACTION_WIRELESS_SETTINGS:
                pa.startPreferencePanel(WirelessSettings.class.getName(), null,
                        R.string.wireless_networks_settings_title, null, null, 0);
                break;
    ...
    private void setItemDetails(ViewGroup wifiItem, String label, String value) {
        ...
        try {
            fontScale = ActivityManagerNative.getDefault().getConfiguration().fontScale;
        } catch (RemoteException e) {
            // If exception happened, set fontScale as large so that views show normal
            fontScale = AccessibilitySettings.getLargeFontScaleConstent();
        }

        // If current fontScale is large and choice is wifi, use one textview to show usage
        if ((Float.floatToIntBits(AccessibilitySettings.getLargeFontScaleConstent()) == Float.floatToIntBits(fontScale))
            && (DrainType.WIFI == mDrainType)) {
