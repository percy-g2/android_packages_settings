...
public class BatteryHistoryChart extends View {
    ...
    void finishPaths(int w, int h, int levelh, int startX, int y, Path curLevelPath,
        ...
        if (lastCharging) {
            mChargingPath.lineTo(w, h-mChargingOffset + mChargingPaint.getStrokeWidth()/2);
        }
        if (lastScreenOn) {
            mScreenOnPath.lineTo(w, h-mScreenOnOffset + mScreenOnPaint.getStrokeWidth()/2);
        }
        if (lastGpsOn) {
            mGpsOnPath.lineTo(w, h-mGpsOnOffset + mGpsOnPaint.getStrokeWidth()/2);
        }
        if (lastWifiRunning) {
            mWifiRunningPath.lineTo(w, h-mWifiRunningOffset + mWifiRunningPaint.getStrokeWidth()/2);
        }
        if (lastWakeLock) {
            mWakeLockPath.lineTo(w, h-mWakeLockOffset + mWakeLockPaint.getStrokeWidth()/2);
        }
        if (mHavePhoneSignal) {
                ...
                if (rec.cmd == BatteryStats.HistoryItem.CMD_UPDATE) {
                    ...
                    if (lastX != x) {
                        ...
                        if (charging != lastCharging) {
                            if (charging) {
                                mChargingPath.moveTo(x, h-mChargingOffset + mChargingPaint.getStrokeWidth()/2);
                            } else {
                                mChargingPath.lineTo(x, h-mChargingOffset + mChargingPaint.getStrokeWidth()/2);
                            }
                        ...
                        if (screenOn != lastScreenOn) {
                            if (screenOn) {
                                mScreenOnPath.moveTo(x, h-mScreenOnOffset + mScreenOnPaint.getStrokeWidth()/2);
                            } else {
                                mScreenOnPath.lineTo(x, h-mScreenOnOffset + mScreenOnPaint.getStrokeWidth()/2);
                            }
                        ...
                        if (gpsOn != lastGpsOn) {
                            if (gpsOn) {
                                mGpsOnPath.moveTo(x, h-mGpsOnOffset + mGpsOnPaint.getStrokeWidth()/2);
                            } else {
                                mGpsOnPath.lineTo(x, h-mGpsOnOffset + mGpsOnPaint.getStrokeWidth()/2);
                            }
                            lastGpsOn = gpsOn;
                        }
                        ...
                        if (wifiRunning != lastWifiRunning) {
                            if (wifiRunning) {
                                mWifiRunningPath.moveTo(x, h-mWifiRunningOffset + mWifiRunningPaint.getStrokeWidth()/2);
                            } else {
                                mWifiRunningPath.lineTo(x, h-mWifiRunningOffset + mWifiRunningPaint.getStrokeWidth()/2);
                            }
                        ...
                        if (wakeLock != lastWakeLock) {
                            if (wakeLock) {
                                mWakeLockPath.moveTo(x, h-mWakeLockOffset + mWakeLockPaint.getStrokeWidth()/2);
                            } else {
                                mWakeLockPath.lineTo(x, h-mWakeLockOffset + mWakeLockPaint.getStrokeWidth()/2);
                            }
