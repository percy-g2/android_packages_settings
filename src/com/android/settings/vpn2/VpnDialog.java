...
class VpnDialog extends AlertDialog implements TextWatcher,
    ...
    // Listen to watch input method's previous action.
    private OnEditorActionListener mEditorActionListener = new OnEditorActionListener() {
        @Override
        public boolean onEditorAction(TextView view, int actionId, KeyEvent event) {
            if (actionId == EditorInfo.IME_ACTION_PREVIOUS) {
                if (view == mDnsServers) {
                    mSearchDomains.requestFocus();
                } else if (view == mRoutes) {
                    mDnsServers.requestFocus();
                } else if (view == mServer) {
                    mName.requestFocus();
                } else if (view ==  mSearchDomains || view == mL2tpSecret
                        || view == mIpsecIdentifier || view == mIpsecSecret) {
    ...
    protected void onCreate(Bundle savedState) {
        ...
        mSearchDomains.setOnEditorActionListener(mEditorActionListener);
        mDnsServers.setOnEditorActionListener(mEditorActionListener);
        mRoutes.setOnEditorActionListener(mEditorActionListener);
        mServer.setOnEditorActionListener(mEditorActionListener);
        mL2tpSecret.setOnEditorActionListener(mEditorActionListener);
        mIpsecIdentifier.setOnEditorActionListener(mEditorActionListener);
        mIpsecSecret.setOnEditorActionListener(mEditorActionListener);
        ...
        if (mEditing) {
            // It must be creating a new vpn if name is not set.
            if (mName.getText().length() == 0) setTitle(R.string.vpn_create);
            else setTitle(R.string.vpn_edit);
