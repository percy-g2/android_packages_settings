...
public class WifiP2pSettings extends SettingsPreferenceFragment
    ...
    private AlertDialog mDeviceNameDialog;

    private TextWatcher mDeviceNameTextWatcher = new TextWatcher() {
        private Button mOKButton;

        @Override
        public void beforeTextChanged(CharSequence s, int arg1, int arg2, int arg3) {
            if (DBG) Log.d(TAG, " beforeTextChanged");
        }
        ...
        public void afterTextChanged(Editable s) {
            if (DBG) Log.d(TAG, " afterTextChanged");
            if (mDeviceNameDialog != null) {
                mOKButton = mDeviceNameDialog.getButton(DialogInterface.BUTTON_POSITIVE);
                if (mOKButton != null) {
                    if (DBG) Log.d(TAG, "okButton setEnabled");
            ...
            public void onClick(DialogInterface dialog, int which) {
                if (which == DialogInterface.BUTTON_POSITIVE) {
                    if (mWifiP2pManager != null) {
                        ...
                        if (mDeviceNameText.getText().toString().length() == 0) {
    ...
    public Dialog onCreateDialog(int id) {
        if (id == DIALOG_DISCONNECT) {
            ...
            mDeviceNameText.setImeOptions(EditorInfo.IME_ACTION_DONE);
            mDeviceNameText.setInputType(EditorInfo.TYPE_CLASS_TEXT);
            ...
            mDeviceNameDialog = new AlertDialog.Builder(getActivity())
                ...
                .setNegativeButton(getActivity().getString(R.string.dlg_cancel), null)
                .create();
            mDeviceNameText.addTextChangedListener(mDeviceNameTextWatcher);
            return mDeviceNameDialog;
        }
        return null;
    }
