...
public class UserDictionaryAddWordFragment extends Fragment
    ...
    // The current LocaleRenderer
    private LocaleRenderer mLocale;
    ...
    public void onConfigurationChanged(Configuration newConfig) {
        ...
        // Only when orientation changed, the softkeyboard need hide.
        if (mOrientation != newConfig.orientation) {
            InputMethodManager imm = (InputMethodManager)
                    getActivity().getSystemService(Context.INPUT_METHOD_SERVICE);
            if (!getActivity().hasWindowFocus() && (imm != null) && (imm.isActive())) {
                imm.hideSoftInputFromWindow(getActivity().getCurrentFocus().getWindowToken()
                        ,InputMethodManager.HIDE_NOT_ALWAYS);
            }
        }
        // Save the new orientation config.
        mOrientation = newConfig.orientation;
        // clear menu item and reload menu item.
        menu.clear();
        MenuItem actionItem = menu.add(0, OPTIONS_MENU_DELETE, 0, R.string.delete)
                .setIcon(android.R.drawable.ic_menu_delete);
        actionItem.setShowAsAction(MenuItem.SHOW_AS_ACTION_IF_ROOM |
    ...
    public void onPause() {
        super.onPause();
        // Avoid to saved automatically when you select "more language".
        if (mLocale !=null && mLocale.isMoreLanguages()) {
            return;
        }
    ...
    public void onItemSelected(final AdapterView<?> parent, final View view, final int pos,
        ...
        if (locale.isMoreLanguages()) {
            ...
            // Hide IME if it's active.
            // Avoid IME still exist when local pick popup.
            ...
            UserDictionarySettings.localeChanged("");
        } else {
            mContents.updateLocale(locale.getLocaleString());
            UserDictionarySettings.localeChanged(locale.getLocaleString());
        }
    }
