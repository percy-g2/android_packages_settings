...
public class ApnEditor extends PreferenceActivity
    ...
    private static String mInitName = null;
    ...
    private boolean mShowPPPNum = false;
    ...
    protected void onCreate(Bundle icicle) {
        ...
        if (intent.getBooleanExtra("fromOptionMenu", false)) {
            setTitle(R.string.apn_new);
        }
    ...
    private void fillUi(String defaultOperatorNumeric) {
        if (mFirstTime) {
            ...
            if (name.contains("ro.")){
                //for china union
                if (ChinaUnionPLMN.equals(MSimTelephonyManager.getDefault().getSimOperator(mSubscription))){
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        // If airplane mode is on, set delete items disable.
    ...
    public boolean onPrepareOptionsMenu(Menu menu) {
        ...
        if (deleteItem != null) deleteItem.setEnabled(enable);
    ...
    private boolean validateAndSave(boolean force) {
        ...
        // we modify the name for some carrier in UI,but no hope to save in db, because db is shared,
        // this fix a sort change issue in apn list after enter some apn edit screen
        if(null!= mInitName && !mInitName.equals(name))
